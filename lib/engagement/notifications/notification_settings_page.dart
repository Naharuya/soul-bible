...
class NotificationSettingsPage extends StatefulWidget {
  const NotificationSettingsPage({super.key, this.startTomorrow = false});
        ...
        builder: (context, _) {
          ...
          Future<void> update(
            ...
            final next = ReminderSettings(
                startOn: widget.startTomorrow
                    ? DateTime(controller.clock().year,
                        controller.clock().month, controller.clock().day + 1)
                    : settings.startOn,
                enabled: enabled ?? settings.enabled,
                hour: time?.hour ?? settings.hour,
                minute: time?.minute ?? settings.minute,
                kind: kind ?? settings.kind,
                gentle: gentle ?? settings.gentle,
                paused: enabled == true ? false : settings.paused);
            final success = await controller.configure(next);
            if (success && next.enabled && !settings.enabled) {
              await engagement.track(EngagementMetric.notificationOptIn);
            }
          ...
          return SpaceScaffold(
              ...
              body: ListView(padding: const EdgeInsets.all(20), children: [
                ...
                if (widget.startTomorrow)
                  const Text('알림을 켜거나 시간을 바꾸면 내일부터 선택한 시간에 매일 알려드려요.'),
                const Text(
                    '알림을 켜지 않아도 모든 기능을 사용할 수 있어요. 대화나 개인 기록은 알림에 넣지 않아요.'),
                if (settings.paused)
                  const Padding(
                ...
                ...ReminderKind.values
                    .where((kind) => kind != ReminderKind.gentle)
                    .map((kind) => ListTile(
                        title: Text(kind.label),
                        leading: Icon(settings.kind == kind
                            ? Icons.radio_button_checked
                            : Icons.radio_button_off),
                        selected: settings.kind == kind,
                        onTap: _busy || !controller.ready
                            ? null
                            : () => update(kind: kind))),
                SwitchListTile(
