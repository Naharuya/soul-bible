...
class SharePreviewPage extends StatefulWidget {
  const SharePreviewPage(
      {super.key, required this.content, this.gateway, this.renderer});
...
class _SharePreviewPageState extends State<SharePreviewPage> {
  ...
  Future<void> _export(BuildContext buttonContext, {bool save = false}) async {
    ...
    try {
      ...
      if (!save && result == CardShareResult.completed) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(save
                ? '이미지를 저장하지 못했어요. 저장 위치를 확인하고 다시 시도해 주세요.'
  ...
  Widget build(BuildContext context) => SpaceScaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
                ...
                child: FutureBuilder<Uint8List>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasError) {
                        return const Text('미리보기를 만들지 못했어요. 뒤로 갔다가 다시 열어 주세요.');
                      }
                      if (!snapshot.hasData) {
        ...
        SwitchListTile(
            title: const Text('생성 날짜 표시'),
            value: _content.date != null,
            onChanged: _busy
                ? null
                : (value) => setState(() {
                      _content = widget.content
                          .withDate(value ? DateTime.now() : null);
                      _image = _renderer.render(_content);
                    })),
        Builder(
        ...
        if (widget.gateway != null || NativeCardShare.supportsSave) ...[
          ...
          Builder(
              builder: (buttonContext) => OutlinedButton.icon(
                  onPressed:
                      _busy ? null : () => _export(buttonContext, save: true),
