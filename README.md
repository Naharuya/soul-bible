...
무료 회원은 하루 3회, 프리미엄 회원은 사용 횟수 제한 없이 대화를 시작할 수
있습니다. 현재 결제·스토어 검증 서버는 아직 연결되지 않았으므로, 개발 중에는
다음 빌드 플래그로 프리미엄 상태를 시뮬레이션합니다.

```bash
flutter run --dart-define=SOUL_BIBLE_PREMIUM_MEMBER=true
```
