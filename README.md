...
말씀과 작은 실천을 제안하는 Flutter MVP 앱입니다. 기존 Bible Mind 핵심
패키지를 보존하면서 실제 모바일 화면과 데모 대화 흐름을 추가했습니다.
...
4. 입력 전 로컬 위기 표현 감지
5. 세 번째 답변 후 감정별 말씀 한 구절 자동 표시
6. 작은 실천 선택
7. 오늘의 마음 카드 전체 문구 기기 저장
8. 109·112·119 전화 연결

기본값은 API 키 없이 전체 흐름을 확인할 수 있는 데모 모드입니다.
...
권한을 요청합니다. 허용하면 한국어 인식 결과가 입력창에 실시간으로 표시됩니다.
...
묵상 질문을 한국어 음성으로 천천히 낭독합니다. 재생 중에는 버튼이 정지 버튼으로
바뀝니다. 마이크 입력과 말씀 낭독은 동시에 실행되지 않도록 서로 자동 정지합니다.
...
```bash
flutter run \
  --dart-define=SOUL_BIBLE_API_URL=http://10.0.2.2:8787/v1/mind/chat
```

Android 에뮬레이터에서는 호스트의 `localhost` 대신 `10.0.2.2`를 사용합니다.
iOS 시뮬레이터와 Web은 보통 `http://localhost:8787/v1/mind/chat`을 사용합니다.
