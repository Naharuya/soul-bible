...
## 프로젝트 규칙
- rami://t/{id} 처리와 app_links 초기 URI/stream 처리를 검증한다.
- NFC read/write 안정성과 잘못된 태그 fallback을 우선한다.
- SoulBible/K-Stock을 수정하지 않는다.
- 실기기 테스트 없이 NFC 완료로 표시하지 않는다.
- 사용자 데이터 삭제를 금지한다.
- Flutter 테스트·분석·플랫폼 빌드와 딥링크 테스트를 변경 범위에 맞게 실행한다. Android/iOS NFC 실기기 검증은 별도로 보고한다.
- 상위 ONARIA 저장소 안에 있어도 RAMI 작업 범위는 이 디렉터리다. 상위의 ONARIA 전용 API·서명·설치 규칙을 RAMI에 적용하지 않는다. 공통 안전 규칙은 유지한다.
- Git 루트가 상위 폴더이면 이번 RAMI 변경만 선택해서 커밋한다. 독립 폴더에 Git이 없으면 자동으로 저장소를 만들거나 다른 앱 remote를 사용하지 않는다.
