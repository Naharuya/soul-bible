import { answerExamplesSchema, answerExamplesJsonSchema, answerExamplesInstructions } from '../answer_examples.js';
...
export const supportJsonSchema = {
  type: 'object', additionalProperties: false, required: ['emotion', 'empathy', 'nextQuestion', 'summary', 'answerExamples'],
  properties: { answerExamples: answerExamplesJsonSchema, emotion: { type: 'string', enum: emotions }, empathy: { type: 'string' },
...
export function createSupportTurn({ runStructured }) {
  return async (context, options) => supportSchema.parse(await runStructured({
    ...
    instructions: '감정 관찰과 짧은 공감, 다음 질문 하나, 핵심 기억 한 문장을 한 번에 작성합니다. 각 문장은 짧게 쓰세요. 요약은 감정과 필요만 담고 원문을 복사하지 마세요. 진단·처방·교리 해석·경전 인용은 하지 마세요. 질문은 사용자가 이미 답한 질문을 반복하지 마세요.' + answerExamplesInstructions,
