...
test('admin usage UI renders measured costs and does not present missing pricing as free', async () => {
  ...
  context.renderAiUsage({ available: true, today: { modelCalls: 2, estimatedCostUsd: 0.01 },
  ...
  assert.equal(elements.get('aiCost').textContent, '—');
  context.renderAiUsage(undefined);
  assert.equal(elements.get('aiCalls').textContent, '—');
